...
import nwd.fokuslauncher.data.model.isAppHiddenByMetadata
import nwd.fokuslauncher.data.model.overlayCustomName
...
import nwd.fokuslauncher.utils.DotSearchParsed
...
import nwd.fokuslauncher.data.search.AppSearchChip
import nwd.fokuslauncher.data.search.AppTargetChips
import nwd.fokuslauncher.data.search.CalendarSearchResult
import nwd.fokuslauncher.data.search.CallLogSearchResult
import nwd.fokuslauncher.data.search.ContactSearchResult
import nwd.fokuslauncher.data.search.FileSearchResult
import nwd.fokuslauncher.data.search.LocalSearchManager
import nwd.fokuslauncher.data.search.MessageSearchResult
import nwd.fokuslauncher.data.search.QuickActionCalculator
import nwd.fokuslauncher.data.search.QuickActionResult
import nwd.fokuslauncher.data.search.SettingsSearchResult
import nwd.fokuslauncher.data.search.SystemSettingsSearch
...
sealed interface LaunchTarget {
    ...
    data class MainApp(override val packageName: String) : LaunchTarget
...
fun launchTargetFromAppInfo(app: AppInfo, privateSpaceManager: PrivateSpaceManager? = null): LaunchTarget {
    ...
    return if (uh != null && cn != null && privateSpaceManager != null && privateSpaceManager.isPrivateSpaceProfile(uh)) {
...
constructor(
    ...
    private fun observeDrawerShowScrollbar() {
        viewModelScope.launch {
            preferencesManager.drawerShowScrollbarFlow.collect { show ->
                _uiState.update { it.copy(drawerShowScrollbar = show) }
            }
        }
    }
    ...
    private fun createCurrentMetadataSnapshot() = DrawerMetadataSnapshot(
    ...
    private fun loadApps() {
        viewModelScope.launch {
            rebuildVisibleApps(createCurrentMetadataSnapshot())
        }
    }
    ...
    fun onSearchQueryChanged(query: String) {
        ...
        searchQueryApplyJob =
                viewModelScope.launch {
                    ...
                    val quickAction = QuickActionCalculator.evaluate(query)
     ...
     */
    fun executeWebSearch(overrideQuery: String? = null): Boolean {
        val q = (overrideQuery ?: _uiState.value.searchQuery).trim()
        if (q.isBlank()) return false
        val browserChip = AppTargetChips.getChips().firstOrNull { it.id == "browser" }
        return browserChip?.launchAction?.invoke(context, q) ?: false
    }
    ...
    fun tryLaunchFirstSearchResult(): Boolean {
        ...
        if (firstMain != null) {
            return launchTarget(launchTargetFromAppInfo(firstMain, privateSpaceManager))
        }
    ...
    private fun buildLaunchTargets(
        ...
        val mainTargets = mainApps.map { launchTargetFromAppInfo(it, privateSpaceManager) }
    ...
    private fun mergeOrderListForProfile(
        ...
        val prev = map[profileKey].orEmpty()
